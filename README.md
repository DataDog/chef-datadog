...
been blacklisted in cookbook versions `>3.3.0`. `>2.20.0`.

*PLEASE NOTE:* chef-runs will fail on windows when updating to cookbook versions `>3.3.0`. `>2.20.0`
if:
...
1. Install the latest datadog cookbook on your Chef Server (`>3.3.0` or `>2.20.0`)
...
datadog cookbook (`>3.3.0` or `>2.20.0`), before setting the `agent6_version` attribute. This is a
