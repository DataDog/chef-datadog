...
* With Chef >= 12.6 _and_ the `windows` cookbook `>= 1.39.0` and `< 3.0`, Agent upgrades are known to fail.
  For Chef>=12.6 users on Windows, we recommend pinning the `windows` cookbook to either:
  * a `>= 3.0` version, or
  * a lower `1.x` version (`~> 1.38.0` for instance)
